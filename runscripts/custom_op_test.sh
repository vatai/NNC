#!/bin/sh
# Build the custom op and run custom_op_test.py.
...
make $MAKEFLAG -C src/unsortOp/ && LD_LIBRARY_PATH=/opt/cuda/targets/x86_64-linux/lib/ CUDA_VISIBLE_DEVICES=$GPUs $OPTIRUN python src/custom_op_test.py
